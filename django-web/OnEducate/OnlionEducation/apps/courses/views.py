...
class CourseListView(View):
    ...
    def get(self, request):
        ...
        if search_keywords:
            #name__icontains（name包含search_keyword）
...
class CourseInfoView(LoginRequiredMixin, View):  # 继承LoginRequiredMixin，限制为登录后
    """
    课程章节信息
    """
    def get(self, request, course_id):
        course = Course.objects.get(id=int(course_id))
        course.students += 1  # 点击浏览量加一
        course.save()
        #查询用户是否已经关联了该课程
        ...
        all_user_courses = UserCourse.objects.filter(user_id__in=user_ids)  # user_ids在user_id__in即返回
        #取出所有课程id
...
class CommentsView(LoginRequiredMixin, View):  # 继承LoginRequiredMixin，限制为登录后
    """
    课程评论页
    """
    def get(self, request, course_id):
